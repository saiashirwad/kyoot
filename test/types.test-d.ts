...
type _Merge = Expect<
  Equal<Merge<{ abort: TypeError }, { abort: RangeError; var: number }>, { abort: TypeError | RangeError; var: number }>
>
...
type _AfterAbort = Expect<Equal<typeof afterAbort, KyoT<Result<Error, number>, { var: number; emit: string }>>>

const afterVar = prog.pipe(Var.run(0))
type _AfterVar = Expect<Equal<typeof afterVar, KyoT<[number, number], { abort: Error; emit: string }>>>

const afterEmit = prog.pipe(Emit.run())
type _AfterEmit = Expect<Equal<typeof afterEmit, KyoT<[number, string[]], { abort: Error; var: number }>>>
...
type _Deep = Expect<
  Equal<
    typeof d10,
    KyoT<number, { var: number; emit: string | number; sync: true; abort: string | TypeError; async: true }>
  >
>

// --- the full pipeline at the edge ------------------------------------------

const atTheEdge = d10.pipe(
  Var.run(0),
  Emit.discard(),
  Sync.run(),
  Abort.run(),
