...
export function race<A, S1 extends Row, S2 extends Row>(
...
): Kyoot<A, Merge<Merge<S1, S2>, { async: true }>> {
  return asyncOp({
    execute: (rt) => {
  ...
  }) as Kyoot<A, Merge<Merge<S1, S2>, { async: true }>>;
}
...
): Kyoot<ReadonlyArray<A>, Merge<S, { async: true }>>;
