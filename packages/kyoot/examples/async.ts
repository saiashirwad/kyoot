...
const main = Kyoot.gen(function* () {
  ...
  const starters = yield* Async.all(
    ["bulbasaur", "charmander", "squirtle"].map((n) => pokemon(n).pipe(Fail.run)),
  );
  const ditto = yield* Async.timeout(5000, pokemon("ditto").pipe(Fail.run)).pipe(Fail.run);
