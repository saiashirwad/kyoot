...
test('fiber.await returns a Result instead of throwing', async () => {
  ...
  const prog = Kyoot.gen(function*() {
    const fiber = yield* Async.fork(Async.sleep(1).map(() => {
      throw boom
    }))
...
test('timeout: a slow computation fails with a typed TimeoutError', async () => {
  ...
  assert.equal(r.ok, false)
  assert.ok(Result.isErr(r) && r.cause._tag === 'Fail' && r.cause.error instanceof Async.TimeoutError)
})

test('timeout: a fast computation wins', async () => {
  const r = await Kyoot.runPromise(
    Async.timeout(50, Async.sleep(5).map(() => 'quick')).pipe(Abort.orThrow()),
  )
...
test('runPromise on an unhandled non-async effect rejects loudly', async () => {
  ...
  await assert.rejects(Kyoot.runPromise(k as never), (e: unknown) =>
    e instanceof Error && e.message.includes("unhandled effect 'abort'"),
  )
})

test('runSync rejects an async op at runtime', () => {
  assert.throws(() => Kyoot.runSync(Async.sleep(1) as never), (e: unknown) =>
    e instanceof Error && e.message.includes("unhandled effect 'async'"),
  )
})
